...
export default function HomeScreen() {
  ...
  // Card height: screen - header (no tab bar with drawer navigation)
  const HEADER_HEIGHT = 50;
  const CARD_HEIGHT = SCREEN_HEIGHT - HEADER_HEIGHT - insets.top - insets.bottom;
  ...
  // Load preferences on mount - uses reliable AsyncStorage
  useEffect(() => {
    const loadPreferences = async () => {
      try {
        // PRIORITY 1: Categories passed directly from onboarding
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      {/* Header with Hamburger Menu and Category Pills */}
